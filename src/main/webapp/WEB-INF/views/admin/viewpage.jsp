...
<c:when test="${listValue.status>=1}">
	<tr>
 <td>${listValue.userId}</td><td>${listValue.lname}</td> <td>${listValue.fname}</td><td><a href="viewst/${listValue.userId}">View / Edit</a></td>
 <td><form action="deletes/${listValue.userId}" method="post"><button id="submit" name="submit" class="btn btn-danger" Onclick="return ConfirmDelete();">Delete</button></form></td>
 </tr>
 </c:when>
 </c:choose>
</c:forEach>
